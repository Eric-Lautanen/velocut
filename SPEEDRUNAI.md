...
**`ActiveBlend` struct** (pb thread internal): `{ spec: PlaybackTransitionSpec, aspect: f32, decoder_b: Option<LiveDecoder> }` — `aspect` added this session (inherited from `StartBlend` command, intended to match decoder_b output dims to primary). `decoder_b` is always opened **lazily** at blend zone entry in the frame loop. **No eager/async pre-open**: `LiveDecoder` contains `ffmpeg::software::scaling::Context` (`*mut SwsContext`) which is not `Send` — cannot cross thread boundaries. Lazy open uses `skip_until_pts` (decode-only, ~4× faster than `burn_to_pts`) for decoder_b only; this is safe because decoder_b frames are never checked against `poll_playback`'s lower-bound timestamp gate. **⚠ NOTE: `aspect` passed to `LiveDecoder::open` for decoder_b is currently dead/ignored** — `LiveDecoder::open` always outputs native AR (`out_h=640*src_h/src_w`). Passing project AR has no effect. Dimension mismatch between primary and decoder_b on mixed-AR timelines is therefore still not fixed at the decoder level; fix must be in the blend step (crop decoder_b output to primary dims before blending — see unresolved bugs below).
Pb thread blend logic: `StartBlend` handler sets `blend = Some(ActiveBlend { spec, aspect, decoder_b: None })` then opens+burns the **primary** decoder (burn_to_pts, same as Start). In frame loop: `defer_decoder_b` guard added — if `decoder_b.is_none() && pb_frame_tx.len() < 6`, skip blend this frame (send raw primary) to buffer headroom before absorbing decoder_b open latency. When `ts_secs >= spec.blend_start_ts`, `alpha = (alpha_start + local_t/duration).clamp(0,1)`. Lazy decoder_b open: set `db.skip_until_pts = tpts` then push to `b.decoder_b`. Dimension guard: if `data_b.len() != data.len() || wb != w || hb != h` → log + fall through to unblended frame. `decoder_b_exhausted` flag (set inside mutable-borrow closure, acted on after closure returns) clears `blend` when decoder_b hits EOF. If `spec.invert_ab`, calls `blend_rgba_transition(data_b, data, ...)` else `blend_rgba_transition(data, data_b, ...)`. Blend cleared on Start/Stop/primary-EOF/decoder_b-EOF.
...
**`alpha_start` hardcoded 0.5 causing sync drift on late clip_changed** — If tick() fired 30ms after the cut point, blend would start at alpha=0.5 ignoring elapsed time, causing a tiny visual discontinuity at the join. Fix: `alpha_start = clamp(0.5 + elapsed/duration, 0.5, 1.0)`.
...
**✅ RESOLVED — `alpha_start` hardcoded 0.5 ignoring actual elapsed time** — If `clip_changed` fired late (e.g. 30ms into clip_b), blend would start at alpha=0.5 even though the transition had already progressed. Fix: `alpha_start = clamp(0.5 + elapsed/duration, 0.5, 1.0)` dynamic.

**⚠ UNRESOLVED — clip_b same-AR: video freezes, audio continues** — Confirmed via logs: `StartBlend` received, `build_incoming_blend_spec` returns `Some`, `start_blend_playback` called. But video freezes immediately while audio plays on. Root cause: `defer_decoder_b` (threshold `pb_frame_tx.len() < 6`) defers the decoder_b **open** but not the **skip cost**. On the first `db.next_frame()` call after open, `skip_until_pts` has to decode-only through `clip_a_tail` frames (e.g. `~5s × 30fps = ~150 frames decode-only`). Even at ~1ms/frame this is ~150ms blocking the pb thread with nothing new being sent to pb_frame_tx → UI starves → video freeze. The 6-frame buffer is consumed before skip completes.
**Fix direction**: The skip happens lazily inside `next_frame()`. The correct fix is to send raw primary frames throughout the skip (not just during open). Track whether decoder_b is still in skip-mode: add a helper or flag that indicates `db.skip_until_pts > 0 && db.last_pts < db.skip_until_pts` and treat that as another defer condition in the blend loop — keep sending raw primary until decoder_b has caught up. Alternatively, since decoder_b is the SECONDARY (clip_a tail at ~D/2 duration), open it at the keyframe nearest `clip_a_tail` without any skip. The first few decoder_b frames will be slightly before the ideal position but visually negligible at transition mid-point.
**Recommended fix**: In the blend phase-2 closure, after opening decoder_b, check `if b.decoder_b.as_ref().map(|db| db.skip_until_pts > 0).unwrap_or(false) { return None; }` — this sends raw primary frames during the entire skip window, not just during open.

**⚠ UNRESOLVED — clip_b different-AR: jump cut after clip_a transition** — Confirmed via logs: `[pb] blend size mismatch — primary 320×176 vs decoder_b 320×214; skipping blend` on every frame for the full blend zone. Spec includes `aspect: f32` in `ActiveBlend` and `decoder_b_aspect` is passed to `LiveDecoder::open`, but **`aspect` param is dead/ignored in `LiveDecoder::open`** — output is always native AR (`out_h = 640 * src_h / src_w`). This is intentional architecture (see invariant: consumers crop themselves). Fix must be in the **blend step**, not the decoder.
**Fix direction**: In the blend phase-2 closure, after `db.next_frame()` returns `(data_b, wb, hb, _)`, if `wb != w || hb != h`, apply center-crop math to `data_b` to produce a `w × h` RGBA buffer before calling `blend_rgba_transition`. The crop math is identical to `crop_uv_rect` in `preview_module.rs` but applied to raw RGBA pixels: compute `src_ar = wb/hb`, `dst_ar = w/h`, crop the wider dimension. A helper `crop_rgba(data, src_w, src_h, dst_w, dst_h) -> Vec<u8>` belongs in `worker.rs` alongside `blend_rgba_transition`. Only allocates when dims actually differ — same-AR clips hit the existing fast path unchanged.
...
- Transition preview: scrub blend correct (both halves). Live playback blend: clip_a half works correctly; clip_b half has two remaining bugs: (1) same-AR: video freezes on clip_b start while audio continues — `skip_until_pts` blocking pb thread; (2) different-AR: jump cut after clip_a half — decoder_b output dims mismatch primary because `LiveDecoder::open` ignores `aspect` param (see unresolved section for fix directions).

...
**`ActiveBlend` struct** (pb thread internal): `{ spec: PlaybackTransitionSpec, decoder_b: Option<LiveDecoder> }` — replaces the raw `(spec, Option<LiveDecoder>)` tuple. `decoder_b` is always opened **lazily** at blend zone entry in the frame loop. **No eager/async pre-open**: `LiveDecoder` contains `ffmpeg::software::scaling::Context` (`*mut SwsContext`) which is not `Send` — cannot cross thread boundaries. Lazy open uses `skip_until_pts` (decode-only, ~4× faster than `burn_to_pts`) for decoder_b only; this is safe because decoder_b frames are never checked against `poll_playback`'s lower-bound timestamp gate.
Pb thread blend logic: `StartBlend` handler sets `blend = Some(ActiveBlend { spec, decoder_b: None })` then opens+burns the **primary** decoder (burn_to_pts, same as Start). In frame loop: when `ts_secs >= spec.blend_start_ts`, `alpha = (alpha_start + local_t/duration).clamp(0,1)`. Lazy decoder_b open: set `db.skip_until_pts = tpts` then push to `b.decoder_b`. Dimension guard: if `data_b.len() != data.len() || wb != w || hb != h` → log + fall through to unblended frame (prevents panic on mixed-AR clips). `decoder_b_exhausted` flag (set inside mutable-borrow closure, acted on after closure returns) clears `blend` when decoder_b hits EOF. If `spec.invert_ab`, calls `blend_rgba_transition(data_b, data, ...)` else `blend_rgba_transition(data, data_b, ...)`. Blend cleared on Start/Stop/primary-EOF/decoder_b-EOF. Bug fix: primary open failure in StartBlend (both active and idle branches) now also clears `blend` — previously left orphaned blend state with no primary decoder.
**Instrumentation** (all `eprintln!`): Start/StartBlend received with ts, primary burn elapsed ms, frame #N every 60 frames with ts, primary EOF.
...
**decoder_b EOF not clearing blend state** — after decoder_b exhausted its D/2 of content, `blend` was never set to `None`. Every subsequent clip_b frame re-entered the blend branch and called `next_frame()` on a dead FFmpeg decoder, stalling or spinning the pb thread. Fix: `decoder_b_exhausted` flag set inside the mutable-borrow closure, `blend = None` applied after closure returns.
...
**⚠ UNRESOLVED — clip_b half of transition hard-cuts instead of blending** — Clip_a transition half works correctly. When the playhead crosses into clip_b's timeline range, `clip_changed` fires in `tick()`, `build_incoming_blend_spec` should return `Some` and `start_blend_playback` should be called. Instead, playback jumps directly to clip_b with no blend (appears as if `start_playback` is called). Clips remain playable after the failed transition. **Current hypotheses (in priority order):**
1. **`build_incoming_blend_spec` returns `None` unexpectedly** — the direct time guard `current_time >= clip_b.start_time + half_d` may already be true by the time `clip_changed` fires (stable_dt overshot, or clip_b is very short). Check: add `eprintln!` at the top of `build_incoming_blend_spec` logging `current_time`, `clip_b.start_time`, `half_d`, and the guard result.
2. **`clip_changed` fires a tick late** — `playback_media_id` is compared against `clip_at_time(state, current_time).media_id`. If `current_time` is already past the incoming zone before the comparison runs, `build_incoming_blend_spec`'s time guard fails and `build_blend_spec` returns `None` too (no outgoing transition on clip_b), resulting in plain `start_playback`. Check: log `current_time` and `clip_b.start_time` at the moment `clip_changed` is detected.
3. **`build_incoming_blend_spec` cannot find the preceding clip_a** — the clip_a lookup iterates V-row clips and finds the one whose `start_time + duration` is nearest to `clip_b.start_time`. If there is a small gap between clip_a end and clip_b start (floating-point snap error), the lookup may fail. Check: log the gap.
4. **`start_blend_playback` try_send drops the command** — pb_cmd_rx cap=4; if a Stop + Start from clip_a's playback is still pending, StartBlend gets dropped. The new logging should surface this immediately.
**Recommended next step**: request `video_module.rs` and add `eprintln!` at: (a) entry of `build_incoming_blend_spec` with all guard values, (b) the `clip_changed` detection point in `tick()` showing `current_time` and `clip_b.start_time`, (c) whether `start_blend_playback` or `start_playback` is ultimately called, (d) check the existing StartBlend-received log to confirm the pb thread actually gets the command.
...
- Transition preview: scrub blend correct (both halves). Live playback blend: clip_a half works correctly; clip_b half hard-cuts instead of blending (unresolved — see Debugging History). Clips remain playable after failed transition.

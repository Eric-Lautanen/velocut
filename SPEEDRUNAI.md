...
| File | What it does |
|------|-------------|
| `lib.rs` | Crate root — `pub mod` for `commands`, `helpers`, `media_types`, `state`, `transitions`. No logic. |
...
| `modules/timeline.rs` | `TimelineModule` — toolbar, ruler, clip rendering, trim handles, DnD drop zone, transition badges, volume/fade popup, hotkey popup, playhead. |

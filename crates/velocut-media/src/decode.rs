...
fn ensure_cpu_frame(frame: ffmpeg::util::frame::video::Video) -> ffmpeg::util::frame::video::Video {
    unsafe {
...
impl LiveDecoder {
    ...
    pub fn open(
        ...
        let scaler = match cached_scaler {
            ...
            _ => {
                let sws_src_fmt = if dec_fmt == Pixel::None {
                    Pixel::YUV420P
                } else {
                    dec_fmt
                };
...
unsafe fn center_crop_and_scale_cached(
    ...
    let cache_hit = crop_sws.0.as_ref().is_some_and(|(cw, ch, cf, dw, dh, _)| {
    ...
    if !cache_hit {
        ...
        if let Some((_, _, _, _, _, old)) = crop_sws.0.take() {
            if !old.0.is_null() {
                ffi::sws_freeContext(old.0);
            }
        }
        let pixel_fmt = if fmt_i == nv12 {
            ffi::AVPixelFormat::AV_PIX_FMT_NV12
        } else if fmt_i == p010le {
            ffi::AVPixelFormat::AV_PIX_FMT_P010LE
        } else if fmt_i == yuv420p {
            ffi::AVPixelFormat::AV_PIX_FMT_YUV420P
        } else {
            ffi::AVPixelFormat::AV_PIX_FMT_YUVJ420P
        };
...
pub fn decode_frame(
        ...
        while decoder.receive_frame(&mut decoded).is_ok() {
            ...
            if scaler.is_none() {
                ...
                match SwsContext::get(
                    ...
                    Err(e) => {
                        return Err(anyhow::anyhow!("SwsContext::get failed: {e}"));
                    }
                }
            }
            ...
            if let Some(pts) = decoded.pts() {
                ...
                if pts_secs < timestamp - (1.0 / 60.0) {
                    continue;
                }
            }
    ...
    if let Some(out_frame) = last_good {
        if out_w == 0 {
            return Err(anyhow::anyhow!("no frame found at t={timestamp:.3}"));
        }
        emit_frame(&out_frame, id, out_w, out_h, save_png, &dest, tx)?;
        return Ok(());
    }
...
fn emit_frame(
    ...
    if save_png {
        ...
        let data: Vec<u8> = (0..out_h as usize)
            ...
            .copied()
            .collect();
        let _ = tx.send(MediaResult::VideoFrame {
            id,
            width: out_w,
            height: out_h,
            data,
...
pub fn decode_one_frame_rgba(path: &PathBuf, ts: f64, aspect: f32) -> Result<(Vec<u8>, u32, u32)> {
    ...
    for (stream, packet) in ictx.packets().flatten() {
        ...
        while decoder.receive_frame(&mut decoded).is_ok() {
            ...
            let pts_ok = decoded
                .pts()
                .is_none_or(|pts| pts as f64 * tb_num / tb_den >= ts - (1.0 / 60.0));

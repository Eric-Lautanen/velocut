...
use crate::theme::{
    ACCENT, ACCENT_DUR, DARK_BG_0, DARK_BG_2, DARK_BG_3, DARK_BG_4, DARK_BORDER, DARK_TEXT,
    DARK_TEXT_DIM, SEL_CHECK, SEL_MULTI,
};
                    ...
                    for row in state.library.chunks(cols) {
                        ui.horizontal(|ui| {
                            ...
                            for clip in row {
                                ...
                                let interact =
                                    ui.interact(card_resp.rect, item_id, Sense::click_and_drag());
                ...
                ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                    ...
                    if ui.add(btn).clicked() {
                        if let Some(paths) = FileDialog::new()
                            .add_filter(
                                "Media",
                                &["mp4", "mov", "mkv", "avi", "mp3", "wav", "webm", "m4v"],
                            )
                            .pick_files()
                        {
                            for path in paths {
        ...
        .show(ui, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ...
                if let Some(tex) = thumb_cache.get(&clip.id) {
                    ...
                    ui.painter()
                        .rect_filled(rect, 3.0, Color32::from_rgb(14, 14, 20));
                    ui.painter().text(
                        rect.center(),
                        egui::Align2::CENTER_CENTER,
                        clip_icon(clip.clip_type),
                ...
                ui.add_space(4.0);
                ui.add(
                    egui::Label::new(RichText::new(clip.name.as_str()).size(10.0).color(name_col))
                        .truncate(),
                );
...
fn context_menu(
    ...
    // Clip info header
    ui.label(
        RichText::new(truncate(&clip.name, 28))
            .size(10.5)
            .color(DARK_TEXT),
    );
    ...
    if is_sel && multi_count > 1 {
        ...
        if ui
            .button(format!("🗑  Remove {multi_count} clips"))
            .clicked()
        {
...
fn paint_drag_ghost(
    ...
    // Shadow
    p.rect_filled(
        rect.translate(egui::vec2(3.0, 4.0)),
    ...
    let thumb_rect = rect
        .shrink2(egui::vec2(5.0, 3.0))
        .with_max_y(rect.max.y - 16.0);
...
fn empty_state(ui: &mut Ui) {
    ...
    ui.vertical_centered(|ui| {
        ...
        ui.label(
            RichText::new("or click  + Import  above")
                .size(10.5)
                .color(Color32::from_gray(62)),
        );
        ui.add_space(16.0);
        ui.label(
            RichText::new("Supports MP4, MOV, MKV, AVI, MP3, WAV")
                .size(9.0)
